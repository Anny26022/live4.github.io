...
if not result_df.empty:
    ...
    market_hours = len(filtered_df[filtered_df['Announcement Time'] == "During Market Hours (9:07 AM - 3:30 PM)"])
    after_hours = len(filtered_df[filtered_df['Announcement Time'] == "After Market Hours (3:30 PM - 9:07 AM)"])
    ...
    market_hours_df = filtered_df[filtered_df['Announcement Time'] == "During Market Hours (9:07 AM - 3:30 PM)"].sort_values('NEWS_DT', ascending=False)
    if not market_hours_df.empty:
        ...
        st.dataframe(
            market_hours_df[['NEWS_DT', 'NSE_SYM', 'SLONGNAME', 'HEADLINE', 'PDF']],
            column_config={
    ...
    after_hours_df = filtered_df[filtered_df['Announcement Time'] == "After Market Hours (3:30 PM - 9:07 AM)"].sort_values('NEWS_DT', ascending=False)
    if not after_hours_df.empty:
        ...
        st.dataframe(
            after_hours_df[['NEWS_DT', 'NSE_SYM', 'SLONGNAME', 'HEADLINE', 'PDF']],
            column_config={
                'NEWS_DT': st.column_config.DatetimeColumn('Date & Time', format="DD-MM-YYYY hh:mm a"),
                'NSE_SYM': st.column_config.TextColumn('Symbol'),
                'SLONGNAME': st.column_config.TextColumn('Company'),
